...
![轻量级标签和含附注标签](pic/1240-20210115032519134.png)
...
```
![查看标签](pic/1240-20210115032519132.png)
### 2、管理标签
#### 1）新建轻量级标签
```shell
...
```
![新建轻量级标签](pic/1240-20210115032518837.png)
...
![只有含附注的标签真正建立了tag对象](pic/1240-20210115032518897.png)
...
I、假设当前有两个分支master和dev，现在HEAD指向master分支（即master是当前分支），如下图所示。![当前状态](pic/1240-20210115032518875.png)

II、当前分支（master分支）进行一次提交后，如下图所示。![master分支发生新的提交](pic/1240-20210115032518922.png)

III、执行`git checkout dev`切换分支，结果如图。![切换分支](pic/1240-20210115032518840.png)

### 2、分支命令
> 假设当前HEAD指向唯一的分支master，且当前有两个提交，如下图所示。

![初始状态](pic/1240-20210115032518958-0652318.png)
#### 1）查看分支
```shell
git branch -l # 查看本地分支
...
```
![创建分支](pic/1240-20210115032518925.png)
![切换分支/检出分支](pic/1240-20210115032518926.png)

#### 3）切换分支
> git checkout master

![切换分支/检出分支](pic/1240-20210115032518958.png)
#### 4）合并分支
经过一系列提交后，git空间状态如下图所示。![当前状态](pic/1240-20210115032518875.png)
...
a、git首先会对要合并的两个分支的末端的提交和它们的分离处（共同祖先）的提交进行一次简单的三方合并计算。![三方合并计算](pic/1240-20210115032519300.png)
...
总之，merge commit操作就是根据三个commit对象的快照信息，计算一个新的合并快照，于此同时新建commit对象完成合并操作。![合并结果](pic/1240-20210115032519349.png)

##### II、Fast forward（快进）

a、`git checkout dev`     # 检出dev分支

![检出dev分支](pic/1240-20210115032519227.png)

此时，dev和master两个分支的末端有通路，即有连通的引用链。若在dev分支执行合并master操作，无需创建新的commit对象，可直接将dev的分支指向两个分支中的那个最远末端的commit即可（也就是所谓的快进操作）。

b、`git merge master`

![合并结果](pic/1240-20210115032518985.png)
```shell
# 常用参数
...
![当前状态](pic/1240-20210115032519163.png)



`git rebase master`  # 当前分支衍合至master分支

![衍合分支](pic/1240-20210115032519170.png)
```shell
# 常用参数
