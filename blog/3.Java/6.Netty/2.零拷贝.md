...
<img src="pic/jdpkVNoaeGbIcUdR.png" alt="Image for post" style="zoom: 67%;" />

* 使用sendfile

<img src="pic/R8AZ-MG5UhjYpgfD.png" alt="Image for post" style="zoom:67%;" />
...
![Traditional data copying approach](pic/figure1.gif)

* 使用sendfile时，发生了3次拷贝，用户态拷贝为零。（如果网络接口支持*gather operations*，可以再减少一次拷贝: Read buffer-> Socket buffer）

![Data copy with transferTo()](pic/figure3.gif)

* 不实用sendfile时，发生了四次上下文切换

![Traditional context switches](pic/figure2.gif)

* 使用sendfile时，只有两次上下文切换

![Context switching when using transferTo()](pic/figure4.gif)
...
<img src="pic/POBN-YXYgKS8ZHsn.png" alt="Image for post" style="zoom:67%;" />
